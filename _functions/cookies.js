/**
 * GESTIONNAIRE DE COOKIES RGPD - VERSION DEFINITIVE
...
class CookieManager {
    ...
    /**
     * Supprime un cookie (public, prototype)
     * @param {string} name
     */
    deleteCookie(name) {
    ...
    attachEvents() {
        // Event listener pour le bouton de préférences si il existe
        ...
        banner.innerHTML = `
            <div class="cookie-banner-content">
                <div class="cookie-banner-text">
                    ...
                    <p>Ce site utilise des cookies pour améliorer votre expérience de navigation, analyser l'utilisation du site et personnaliser le contenu.</p>
                </div>
    ...
    showPreferences() {
        const modal = document.getElementById('cookie-preferences-modal');
        if (modal) {
            // Mettre à jour les checkboxes avec les préférences actuelles
    ...
    revokeConsent() {
        ...
        // Envoyer la révocation au serveur
        this.sendConsentToServer(false, this.preferences);
...
function initializeCookieManager() {
    ...
    try {
        ...
        console.log('🔍 Type de l\'instance:', typeof window.cookieManager);
        console.log('🔍 Méthodes disponibles:', Object.getOwnPropertyNames(Object.getPrototypeOf(window.cookieManager)));
        
        // Vérifier que les éléments DOM sont créés
        setTimeout(() => {
            const modal = document.getElementById('cookie-preferences-modal');
            const indicator = document.getElementById('cookie-indicator');
            
            if (modal && indicator) {
                console.log('✅ Éléments DOM créés avec succès');
            } else {
                console.warn('⚠️ Certains éléments DOM manquent');
            }
        }, 100);
...
// Tentative d'initialisation selon l'état du DOM
...
// --- PATCH pour garantir la présence de deleteCookie sur le prototype ---
if (typeof CookieManager !== 'undefined' && typeof CookieManager.prototype.deleteCookie !== 'function') {
...
// --- PATCH pour garantir l'instance globale ---
if (!window.cookieManager || !(window.cookieManager instanceof CookieManager)) {
    window.cookieManager = new CookieManager();
}
...
// Initialisation de secours après un délai
setTimeout(() => {
    if (!window.cookieManager) {
