...
// Déterminer quelle table utiliser (user ou users)
$table_name = 'user';
try {
    $DB->query("SELECT 1 FROM users LIMIT 1");
    $table_name = 'users';
} catch (Exception $e) {
    // La table users n'existe pas, utiliser user
    $table_name = 'user';
}
    ...
    try {
        switch ($_POST['action']) {
            ...
            case 'change_role':
                ...
                $stmt = $DB->prepare("UPDATE $table_name SET role = ? WHERE id = ?");
...
try {
    ...
    $has_telephone = false;
    try {
        $result = $DB->query("SHOW COLUMNS FROM $table_name LIKE 'telephone'");
        $has_telephone = $result->rowCount() > 0;
    } catch (Exception $e) {
        // Ignorer l'erreur
    }
    
    // Construire la requête SELECT en fonction des colonnes disponibles
    $select_fields = "id, pseudo, mail, role, date_creation";
    if ($has_telephone) {
        $select_fields = "id, pseudo, mail, telephone, role, date_creation";
    }
    
    $stmt = $DB->query("SELECT $select_fields FROM $table_name ORDER BY date_creation DESC");
    $users = $stmt->fetchAll();
} catch (Exception $e) {
    $users = [];
}
            ...
            <tbody>
                <?php foreach ($users as $user): ?>
                    <tr data-role="<?php echo $user['role']; ?>" 
                        data-username="<?php echo htmlspecialchars(strtolower($user['pseudo'])); ?>"
                        data-email="<?php echo htmlspecialchars(strtolower($user['mail'])); ?>">
